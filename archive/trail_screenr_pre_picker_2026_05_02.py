#!/usr/bin/env python3
"""
TrailScreenR — Visual polygon reviewer for YOLO trail detections.
...
Usage:
    python3 tools/trail_screenr.py
"""
...
CVAT_USER = "bherwig2"
CVAT_TASK_ID = 15  # Greg Meyer Arizona Brightened - v8 slope-match
FRAME_START = 0    # inclusive
FRAME_END = 963    # exclusive (963 frames)
IMG_DIR = Path("/Volumes/T7 Shield/AI Projects/Star Trail CleanR/star trail images/Greg Meyer Arizona Brightened")
MIN_CROP_PAD = 50
CROP_PAD_RATIO = 0.20  # 20% of the longer polygon dimension
STATE_PATH = Path(__file__).parent.parent / "runs" / "polygon_review" / "polygon_review_2026_04_16" / "trail_screenr_state.json"
MARKS_PATH = STATE_PATH.parent / f"bruce_marks_frames_{FRAME_START}_{FRAME_END - 1}.json"
CLAUDE_FLAGS_PATH = STATE_PATH.parent / f"claude_flags_frames_{FRAME_START}_{FRAME_END - 1}.json"
...
NEIGHBOR_COLOR = (255, 180, 100)   # default light blue for neighbor polygons
...
def load_and_analyze(progress_cb=None):
    ...
    # Group shapes by frame so we can find neighbors overlapping each crop.
    shapes_by_frame = {}
    for s in shapes:
        shapes_by_frame.setdefault(s["frame"], []).append(s)
    ...
    for si, shape in enumerate(sorted(shapes, key=lambda s: (s["frame"], s["id"]))):
        ...
        # Find other polygons in the same frame whose bbox overlaps this crop.
        neighbor_polys = []
        for other in shapes_by_frame.get(frame_idx, []):
            if other["id"] == shape["id"]:
                continue
            other_raw = other["points"]
            other_pts = np.array(list(zip(other_raw[0::2], other_raw[1::2])), dtype=np.float32)
            if len(other_pts) < 3:
                continue
            ox_min, oy_min = other_pts[:, 0].min(), other_pts[:, 1].min()
            ox_max, oy_max = other_pts[:, 0].max(), other_pts[:, 1].max()
            if ox_max < x_min or ox_min > x_max or oy_max < y_min or oy_min > y_max:
                continue
            shifted_other = other_pts.copy()
            shifted_other[:, 0] -= x_min
            shifted_other[:, 1] -= y_min
            neighbor_polys.append({
                "server_id": other["id"],
                "pts": shifted_other.astype(np.int32),
            })
...
class TrailScreenR(QMainWindow):
    ...
    def __init__(self, entries, job_id):
        ...
        # Built from the current and neighbor polygons across all entries.
        ...
        # Neighbor-frame overlay toggle
        self.show_neighbor_frames = False
        self._neighbor_img_cache = {}
        # Track last rendered idx for in-place pixmap update during drag
        ...
        self.setWindowTitle("TrailScreenR")
        ...
        shortcuts = [
            ...
            (QKeySequence(Qt.Key_N),          self._toggle_neighbor_overlay),
    ...
    def _build_polygons_by_id(self):
        ...
        for e in self.entries:
            ...
            for n in e.get("neighbor_polys", []):
                nsid = n["server_id"]
                if nsid in self.polygons_by_id:
                    continue
                npts = n["pts"].astype(np.float32).copy()
                npts[:, 0] += e["crop_x"]
                npts[:, 1] += e["crop_y"]
                # Find the matching entry to get its frame_idx, fall back to
                # the current entry's frame (neighbor-by-frame guarantee).
                self.polygons_by_id[nsid] = {
                    "server_id": nsid,
                    "frame_idx": e["frame_idx"],
                    "points": npts,
                    "original_points": npts.copy(),
                    "status": "original",
                    "label": "trail",
                }
                self._frame_to_sids.setdefault(e["frame_idx"], set()).add(nsid)
    ...
    def _build_banner(self):
        ...
        n = len(self.entries)
        sub = QLabel(f"{n} polygons  |  Greg Meyer Arizona Brightened - v8 slope-match (frames {FRAME_START}-{FRAME_END - 1})")
        sub.setStyleSheet(
        ...
        restart_btn.setToolTip("Close and relaunch TrailScreenR")
        ...
        close_btn.setToolTip("Quit TrailScreenR")
    ...
    def _build_toolbar(self):
        ...
        self.neighbor_btn = QPushButton("Neighbors (N)")
        self.neighbor_btn.setCheckable(True)
        self.neighbor_btn.setStyleSheet(edit_btn_style)
        self.neighbor_btn.setToolTip(
            "Show frame N-1 / N+1 faintly underneath. Real trails move "
            "predictably between frames; false positives don't. Press N."
        )
        self.neighbor_btn.clicked.connect(self._toggle_neighbor_overlay)
        row1.addWidget(self.neighbor_btn)
    ...
    def _build_nav_bar(self):
        ...
        hint_left = QLabel(
            ...
            "Space, D, Del = delete   |   A = add   |   T = tighten   |   N = neighbors   |   "
            "Cmd+Z = undo")
    ...
    def build_display_crop(self, entry):
        ...
        # Optional neighbor-frame overlay (faint blend of N-1 / N+1 crops at
        # the same crop window). Drawn UNDER all polygon overlays.
        if self.show_neighbor_frames:
            nb = self._build_neighbor_blend(entry, crop.shape)
            if nb is not None:
                crop = cv2.addWeighted(crop, 0.65, nb, 0.35, 0)
    ...
    # ── Neighbor-frame overlay (QoL 5) ─────────────────────────────────────

    def _build_neighbor_blend(self, entry, crop_shape):
        """Build a faint blend of frame N-1 and N+1 cropped to the same window
        as `entry`. Returns a uint8 BGR array of crop_shape, or None if neither
        neighbor frame is available."""
        ch, cw = crop_shape[:2]
        cx, cy = entry["crop_x"], entry["crop_y"]
        frame_idx = entry["frame_idx"]
        # Find img paths for frame_idx-1 and frame_idx+1 from any entry that
        # references those frames. If none, fall back to alphabetic neighbors
        # under the same image dir.
        nb_imgs = []
        for off in (-1, 1):
            target = frame_idx + off
            path = self._find_image_for_frame(target)
            if path is None:
                continue
            img = self._neighbor_img_cache.get(path)
            if img is None:
                try:
                    img = cv2.imread(path)
                except Exception:
                ...
                if img is None:
                    continue
                # tiny LRU - keep last 6
                self._neighbor_img_cache[path] = img
                if len(self._neighbor_img_cache) > 6:
                    drop_key = next(iter(self._neighbor_img_cache))
                    if drop_key != path:
                        self._neighbor_img_cache.pop(drop_key, None)
            ih, iw = img.shape[:2]
            x0 = max(0, cx)
            y0 = max(0, cy)
            x1 = min(iw, cx + cw)
            ...
            if x1 <= x0 or y1 <= y0:
                continue
            sub = img[y0:y1, x0:x1]
            # Pad to crop_shape if the source frame is smaller / shifted
            padded = np.zeros((ch, cw, 3), dtype=np.uint8)
            dx0 = x0 - cx
            dy0 = y0 - cy
            padded[dy0:dy0 + sub.shape[0], dx0:dx0 + sub.shape[1]] = sub
            nb_imgs.append(padded)
        if not nb_imgs:
            return None
        if len(nb_imgs) == 1:
            return nb_imgs[0]
        return cv2.addWeighted(nb_imgs[0], 0.5, nb_imgs[1], 0.5, 0)

    def _find_image_for_frame(self, frame_idx):
        """Return the absolute path of the image file for frame_idx, or None.
        Looks first in our own entries, then guesses by alphabetic-sibling in
        the same folder (handles the case where the neighbor frame has no
        polygons, so it has no entry)."""
        for e in self.entries:
            if e["frame_idx"] == frame_idx:
                return e["img_path"]
        # Guess via known img_dir + sorted listing
        if not self.entries:
            return None
        any_entry = self.entries[0]
        img_dir = Path(any_entry["img_path"]).parent
        try:
            siblings = sorted(p for p in img_dir.iterdir()
                              if p.suffix.lower() in (".jpg", ".jpeg", ".png", ".tif", ".tiff"))
        except Exception:
            return None
        if 0 <= frame_idx < len(siblings):
            return str(siblings[frame_idx])
        return None
    ...
    def refresh_view(self):
        ...
        crop = self.build_display_crop(entry)
        pix = self.crop_to_pixmap(crop)
        if self._last_displayed_idx == self.current_idx:
            # Same entry - update pixmap in place, preserve zoom/pan
            self.image_view.update_pixmap(pix)
        else:
    ...
    def _edits_path(self):
        return STATE_PATH.parent / f"trail_screenr_edits_frames_{FRAME_START}_{FRAME_END - 1}.json"
    ...
    def _auto_tighten(self):
        ...
        img = self._neighbor_img_cache.get(entry["img_path"])
        if img is None:
            ...
            if img is None:
                ...
                return
            self._neighbor_img_cache[entry["img_path"]] = img
        ih, iw = img.shape[:2]
        ff = poly["points"]
        ...
        if int(bright.sum()) < 20:
            ...
            return
        # Take the LARGEST connected bright component (drops scattered hot
        # pixels and noise specks; preserves the trail).
        ...
        if len(xs) < 4:
            return
        # PCA-aligned tight rectangle around the bright component
        pts_for_rect = np.column_stack([xs, ys]).astype(np.float32)
        rect = cv2.minAreaRect(pts_for_rect)
        box = cv2.boxPoints(rect)  # 4 corners in region-local
        # Convert back to full-frame
        box[:, 0] += x0
        box[:, 1] += y0
        new_pts = box.astype(np.float32)
    ...
    # QoL 5: Neighbor-frame overlay toggle
    def _toggle_neighbor_overlay(self):
        self.show_neighbor_frames = not self.show_neighbor_frames
        if hasattr(self, "neighbor_btn"):
            self.neighbor_btn.blockSignals(True)
            self.neighbor_btn.setChecked(self.show_neighbor_frames)
            self.neighbor_btn.blockSignals(False)
        self.refresh_view()
...
def main():
    ...
    # 1. Build QApplication FIRST so the splash + lock dialog can use it.
    app = QApplication(sys.argv)
    _apply_theme()

    # 2. Single-instance lock BEFORE any heavy work. Prevents accidental
    #    double-clicks from spawning duplicate loaders.
    ...
    if not lock.tryLock(1500):
        QMessageBox.warning(None, "TrailScreenR already running",
                            "Another TrailScreenR is already running.\n\n"
                            ...
                            "TrailScreenR and try again in 10 seconds.)")
        return

    # 3. Show splash immediately so the user gets visual feedback while CVAT
    #    is fetched and ~4,000 polygons are cropped (~60-90 seconds).
    splash = SplashWindow()
    splash.show()
    splash.update_progress("Checking that Docker is running...")

    # 4. Docker check (may auto-start Docker; on cold boot can take 90-120s)
    from _docker_helper import ensure_docker_running
    def _docker_progress(msg):
        splash.update_progress(msg)
        QApplication.processEvents()
    if not ensure_docker_running(progress_cb=_docker_progress):
        splash.close()
        QMessageBox.critical(None, "Docker didn't finish starting",
            "Docker Desktop didn't finish starting in 3 minutes. "
            "TrailScreenR needs CVAT (which runs inside Docker) to load its "
            "data. Please open Docker Desktop manually, wait for it to finish "
            "starting up, then try again.")
        return

    # 5. Load all polygons + crops (slow). Splash updates as we go.
    entries, job_id = load_and_analyze(progress_cb=splash.update_progress)
    print(f"\nLoaded {len(entries)} polygons, ready for review.")

    if not entries:
        splash.close()
        QMessageBox.information(None, "No polygons",
            "No polygons found for this CVAT task / frame range.")
        return

    # 6. Build the main window, then close the splash.
    splash.update_progress("Building the editor window...")
    window = TrailScreenR(entries, job_id)
    splash.close()
    window.show()
