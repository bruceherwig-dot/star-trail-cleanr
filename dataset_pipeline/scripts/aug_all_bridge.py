...
import sys
# HOLD GUARD (added 2026-06-08): the full bridge augmentation is on hold until
# Bruce reviews CVAT task #42 and approves a handful. Run with AUG_GO=1 to proceed.
if os.environ.get("AUG_GO") != "1":
    print("aug_all_bridge.py: HELD. Full batch waits on #42 review. "
          "Set AUG_GO=1 to run.", flush=True)
    sys.exit(0)
...
OUT="/Volumes/T7 Shield/AI Projects/Star Trail CleanR/bridge_fix_tiles_2026_06/aug_all_bridge"
...
# EXCLUDE tiles Bruce removed during #42 review (2026-06-08): Borrego Gomphothere F9 frames
...
_before=len(recs)
recs=[r for r in recs if (r["ds"],r["frame"]) not in REMOVED]
print(f"excluded {_before-len(recs)} removed-in-review trails",flush=True)
...
for ri_idx,rec in enumerate(recs):
    ...
    for th in TILTS:
        ...
        if is_long:
            ...
            _,_,vt=np.linalg.svd(pts-m,full_matrices=False); axis=vt[0]; t=(pts-m)@axis
