...
## v1.992-beta
- **Symmetric Unicode-path safety: writes too, not just reads.** v1.991 fixed reading files with non-ASCII paths (Slovak `Štrba`, German umlauts, French accents, Cyrillic, CJK, etc.). Same Windows bug also affects writing files via `cv2.imwrite` — the hot-pixel map, the saved trail-detection masks (when "save masks" is on), and the foreground-mask the user paints in the mask editor. All three now use a Unicode-safe writer that tries OpenCV first and falls back to Pillow on failure. Net effect: international users with accented folder names now have a fully working app — every read and every write transparently handles their paths.
- **Routed the last few read paths through the robust reader too.** Five direct `cv2.imread` calls were still bypassing the v1.991 fallback chain: foreground-mask read, JPEG EXIF-rotation re-read, hot-pixel-map re-read, and the two image / mask reads in the mask editor. All now go through the same fallback ladder (OpenCV → tifffile for TIFFs → Pillow → retry).
- **PIL fallback now applies EXIF rotation when the caller asks for color.** `cv2.imread` with `IMREAD_COLOR` honors EXIF Orientation on rotated JPEGs; `IMREAD_UNCHANGED` does not. Our PIL fallback now matches that behavior using Pillow's `ImageOps.exif_transpose`, so rotated JPEGs from phones/cameras are oriented correctly even when the file is rescued by PIL instead of OpenCV.
- **Smoke tests grew by 4** to lock in the new write fallback (basic write, PIL fallback when cv2 fails, grayscale/uint16 paths, and a structural check that all production cv2.imwrite call sites now route through `robust_imwrite`). Total smoke suite: 128 tests.

## v1.991-beta
- **Hotfix: paths with non-ASCII characters now read on Windows.** A Slovak tester's run died on the very first frame because the input folder path was `C:\Users\magio\Desktop\Štrba\svetlá\` and Windows' OpenCV uses old file APIs that can't open files whose path contains non-ASCII characters (Slovak `Š`, `ľ`, `á`; same problem hits Czech, German, French, Cyrillic, CJK, every European language with diacritics). The bug fails BEFORE OpenCV even tries to decode the image, which means our v1.99 fallback ladder (cv2 → tifffile → retry) couldn't help — every retry hit the same Unicode-path failure. v1.991 adds Pillow as a third fallback in the chain. Pillow uses Python's normal file APIs which handle Unicode correctly on every platform, so it transparently rescues these reads. The popup never fires for affected users — the run just works. Smoke test added that locks in the rescue behavior. Total smoke suite is now 124 tests.
