...
- **Bundle smoke test now exercises every output format, not just JPG.** The pre-release smoke test that runs in CI used to only check that the bundled app could process a frame with JPG output. That's why the missing-tifffile bug slipped through three releases — the test simply never tried TIFF 16-bit. The smoke test now runs the bundled worker once for each of the three output formats (JPG, TIFF 8-bit, TIFF 16-bit). If any of them can't load its dependencies, the build job fails and the broken bundle never reaches users.
