...
- **(For the curious: there is no v1.94-beta release.)** v1.94 was tagged earlier today with the tifffile fix above. Its CI build failed because the new per-format smoke test had a separate one-frame bug that prevented every job from completing. v1.95 contains the same tifffile fix, plus the smoke-test fix, plus a verification step (the Mac bundle has now been smoke-tested locally against all three output formats before this tag was pushed).
