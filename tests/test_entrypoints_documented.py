...
def test_entry_points_have_a_module_docstring():
    for name in ENTRY_POINTS:
        doc = ast.get_docstring(ast.parse((REPO / name).read_text(encoding="utf-8")))
...
def test_the_worker_contract_is_written_down():
    ...
    app = (REPO / "star_trail_cleanr.py").read_text(encoding="utf-8")[:6000]
    eng = (REPO / "astro_clean_v5.py").read_text(encoding="utf-8")[:6000]
...
def test_the_map_exists_and_is_linked():
    ...
    assert "ARCHITECTURE.md" in (REPO / "CLAUDE.md").read_text(encoding="utf-8"), \
